...
int countLeaft(TreeNode *tree, int c)
{
    int count = c;
    if (tree != NULL)
    {
        ...
        count = countLeaft(tree->left, count);
        count = countLeaft(tree->right, count);
    }
    return count;
}
...
int main()
{
    ...
    int numberLeaft = 0;
    numberLeaft = countLeaft(tree, numberLeaft);
    printf("Number of Leaft: %d\n", numberLeaft);
}